...
# 保留原始顺序的文本块名称
text_blocks = [f's_{i + 1}' for i in range(len(chunks))]
# 记录每个文本块在原始顺序中的索引
original_indices = {chunk: idx for idx, chunk in enumerate(chunks)}

# 绘制折线图
plt.figure(figsize=(10, 6))
# 按排序后的分数绘制，但 x 轴用原始顺序的文本块名称
plt.plot([text_blocks[original_indices[chunk]] for chunk in sorted_chunks], sorted_scores, marker='o', linestyle='-',
         color='b', label='相关性得分')
...
for i in range(min_k):
    ...
    index = original_indices[chunk]
    plt.scatter(text_blocks[index], score, color='green', zorder=5, label='初始选择' if i == 0 else "")
    plt.text(text_blocks[index], score + 0.02, f'{score:.2f}', fontsize=9, ha='center', color='green')

# 标注梯度检查过程
for i in range(min_k, len(sorted_chunks)):
    ...
    prev_index = original_indices[prev_chunk]
    current_index = original_indices[current_chunk]

    # 绘制梯度阈值线
    plt.hlines(threshold, text_blocks[prev_index], text_blocks[current_index], colors='r', linestyles='dashed',
               label='梯度阈值' if i == min_k else "")

    # 标注当前文本块
    if current_score > threshold:
        plt.scatter(text_blocks[current_index], current_score, color='orange', zorder=5,
                    label='加入选择' if i == min_k else "")
        plt.text(text_blocks[current_index], current_score + 0.02, f'{current_score:.2f}', fontsize=9, ha='center',
                 color='orange')
    else:
        plt.scatter(text_blocks[current_index], current_score, color='gray', zorder=5,
                    label='停止选择' if i == min_k else "")
        plt.text(text_blocks[current_index], current_score + 0.02, f'{current_score:.2f}', fontsize=9, ha='center',
                 color='gray')
        break  # 停止选择

# 添加标题和标签
plt.title('文本块选择过程', fontsize=16)
