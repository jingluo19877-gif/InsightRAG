...
from langchain_community.embeddings import ZhipuAIEmbeddings  # 修改为 ZhipuAIEmbeddings
...
# 初始化智谱嵌入模型
evaluator_embeddings = ZhipuAIEmbeddings(
    model="embedding-3",
...
# 创建 ResponseRelevancy 评估指标并指定使用的 LLM 和 embeddings
...
cursor.execute("SELECT question_id FROM ResponseRelevancy_naive_gpt4")
processed_question_ids = set([row[0] for row in cursor.fetchall()])

# 获取数据总行数
if processed_question_ids:
    count_query = "SELECT COUNT(*) FROM naive_question_responses WHERE id NOT IN (%s)" % (','.join(['%s'] * len(processed_question_ids)))
    cursor.execute(count_query, tuple(processed_question_ids))
else:
    count_query = "SELECT COUNT(*) FROM naive_question_responses"
...
async def process_row(row):
    ...
    # 将结果写入数据库
    insert_query = """
    INSERT INTO ResponseRelevancy_naive_gpt4 (question_id, score)
    VALUES (%s, %s)
    """
