...
from ragas.metrics import Faithfulness, LLMContextRecall, NoiseSensitivity
import asyncio
import time
import gc
...
# 创建 LLMContextRecall 评估指标并指定使用的 LLM
...
cursor.execute("SELECT question_id FROM NoiseSensitivity_naive")
...
if processed_question_ids:
    count_query = """
    SELECT COUNT(*) 
    FROM naive_question_responses hqr
    JOIN hybrid_question_responses_deepseek hqrd ON hqr.question = hqrd.question
    WHERE hqr.id NOT IN (%s)
    """ % (','.join(['%s'] * len(processed_question_ids)))
...
async def process_row(row):
    question_id, question, response, relevant_context, reference = row
    ...
    # 将结果写入数据库
    insert_query = """
    INSERT INTO NoiseSensitivity_naive (question_id, score)
    VALUES (%s, %s)
    """
...
async def process_batch(rows):
    semaphore = asyncio.Semaphore(50)  # 限制并发数量为 50
...
while True:
    if processed_question_ids:
        ...
        query = f"""
        SELECT hqr.id, hqr.question, hqr.response, hqr.relevant_context, hqrd.response 
        FROM naive_question_responses hqr
        JOIN hybrid_question_responses_deepseek hqrd ON hqr.question = hqrd.question
        WHERE hqr.id NOT IN ({','.join(['%s'] * len(processed_question_ids))})
