...
from ragas.metrics import Faithfulness, LLMContextPrecisionWithoutReference, LLMContextPrecisionWithReference
import asyncio
import time
import gc
...
# 创建 LLMContextPrecisionWithoutReference 评估指标并指定使用的 LLM
...
cursor.execute("SELECT question_id FROM context_precision_hybrid")
processed_question_ids = set([row[0] for row in cursor.fetchall()])

# 获取数据总行数
if processed_question_ids:
    count_query = "SELECT COUNT(*) FROM hybrid_question_responses WHERE id NOT IN (%s)" % (','.join(['%s'] * len(processed_question_ids)))
    cursor.execute(count_query, tuple(processed_question_ids))
else:
    count_query = "SELECT COUNT(*) FROM hybrid_question_responses"
...
async def process_row(row):
    ...
    # 将结果写入数据库
    insert_query = """
    INSERT INTO context_precision_hybrid (question_id, score)
    VALUES (%s, %s)
    """
...
while True:
    if processed_question_ids:
        ...
        FROM hybrid_question_responses 
