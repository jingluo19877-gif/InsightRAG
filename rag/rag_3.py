...
from .prompt_test3 import GRAPH_FIELD_SEP
...
class RAG:
    ...
    #向系统中插入自定义文本块的功能，包含同步和异步两个版本的插入方法。其核心逻辑是先检查文档和文本块是否已存在于存储中，若不存在则插入新的文档和文本块，同时进行实体提取和知识图谱更新，最后执行插入完成后的操作
    ...
    #用于处理输入的文档字符串列表。其主要功能包括去除重复文档、为新文档生成唯一的 ID 和初始状态、过滤掉已存储的文档，最后将新的唯一文档存储到系统中，并更新其状态为 PENDING。
    ...
    #从处于待处理（PENDING）或处理失败（FAILED）状态的文本块中提取实体和关系，并更新这些文本块的处理状态。整个过程采用批量处理的方式，利用异步并发提高处理效率，同时使用信号量控制并发任务数量。
    ...
    #实现了一个带有独立关键词提取的查询功能。主要分为同步和异步两个版本的函数，其核心流程是先从查询语句中提取高级和低级关键词，然后根据查询模式调用相应的查询函数进行最终查询，并在查询完成后进行一些收尾操作。
    ...
    #异步函数，用于获取指定实体的详细信息。该函数可以从知识图谱图数据库和向量数据库中获取实体的相关信息，并根据参数决定是否包含向量数据库的数据。
