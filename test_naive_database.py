...
rag = LightRAG(
    ...
    kv_storage="MongoKVStorage",  # 确保与实际的键值存储类型一致
    graph_storage="Neo4JStorage",
    vector_storage="MilvusVectorDBStorge",  # 使用支持的向量存储类型
)
...
while True:
    ...
    for question_data in questions:
        ...
        try:
            ...
            insert_cursor.execute("""
                INSERT INTO naive_question_responses (question, response)
                VALUES (%s, %s)
            """, (question, response))
    ...
    file_name = "naive_mode_context.txt"
            ...
            if i < len(record_ids):
                try:
                    ...
                    update_cursor.execute("""
                        UPDATE naive_question_responses
