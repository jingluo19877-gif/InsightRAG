...
class OllamaChat():
    def __init__(self, system_message="你的名字叫做minglog，是一个由骆明开发的大语言模型。",
                 url="http://localhost:6006/api/chat", model_name="qwen2.5:32b"):
        """
    ...
    def chat(self, prompt, message=None, stream=False, system_message=None, **options):
        ...
        try:
            ...
            responses.raise_for_status()  # 检查请求是否成功
        except requests.RequestException as e:
            print(f"请求出错: {e}")
            return '请求出错', self.message
...
# 假设评论数据在这个文件中
file_path = r"E:\毕业设计\douyin_reviews_finall\cleaned_reviews_1_deduplicated.txt"
output_file_path = r"E:\毕业设计\douyin_reviews_finall\cleaned_reviews_1_deduplicated_1.txt"  # 新的文件路径
...
def filter_reviews_with_entities_and_relations(file_path, output_file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        while True:
            batch_reviews = [f.readline().strip() for _ in range(100)]
            # 过滤掉空行
            batch_reviews = [review for review in batch_reviews if review]
            ...
            for review in batch_reviews:
                ...
                print(f"处理进度: {progress:.2f}% ({processed_reviews}/{total_reviews})")
            ...
            # 清空 valid_reviews 列表以释放内存
            valid_reviews.clear()

            # 重置 Chat 对象的对话历史，只保留系统消息
            Chat.message = [Chat.system_message]
